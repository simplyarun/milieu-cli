...
function makeSuccess(body: string): HttpResponse {
  return { ok: true, url: "https://example.com/.well-known/mcp.json", status: 200, headers: {}, body, redirects: [], durationMs: 50 };
}
function make404(): HttpResponse {
  return { ok: false, error: { kind: "http_error", message: "HTTP 404", statusCode: 404, url: "https://example.com/.well-known/mcp.json" } };
}

describe("checkWebMcp", () => {
  beforeEach(() => mockHttpGet.mockReset());
  it("returns pass with valid JSON", async () => {
    mockHttpGet.mockResolvedValue(makeSuccess('{"tools":[]}'));
  ...
  it("returns partial with invalid JSON", async () => {
    ...
    expect((await checkWebMcp("https://example.com")).status).toBe("partial");
  });
  it("returns fail on 404", async () => {
    mockHttpGet.mockResolvedValue(make404());
    expect((await checkWebMcp("https://example.com")).status).toBe("fail");
  });
});
