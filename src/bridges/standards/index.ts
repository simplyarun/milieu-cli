...
export async function runStandardsBridge(ctx: ScanContext): Promise<BridgeResult> {
  ...
  const [sitemapResult, markdownResult, llmsTxtResult, llmsFullTxtCheck, securityTxtCheck, webMcpCheck, a2aAgentCardCheck] = await Promise.all([
  ...
  const checks: Check[] = [
    ...
    mcpResult.check, jsonLdCheck, schemaOrgCheck, securityTxtCheck,
