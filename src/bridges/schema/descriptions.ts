...
/**
 * Check: schema_descriptions
 *
 * A sample of parameters and schema properties should have descriptions.
 * Agents use descriptions verbatim for field understanding.
 *
 * Sampling: up to 30 items total:
 *   - All path params
 *   - Up to 20 query params
 *   - Properties from first 3 requestBody schemas
 */
export function checkDescriptions(
  ...
  for (const [, , op] of ops) {
    ...
    // Properties from requestBody schemas (first 3)
  ...
  if (sampled === 0) {
    return {
      ...
      label,
      status: "pass",
      detail: "No parameters or properties to evaluate",
      data: { sampled: 0, withDescription: 0 },
    };
  }
  ...
  if (ratio >= 0.8) {
    return {
      ...
      label,
      status: "pass",
      detail: `${withDescription} of ${sampled} sampled fields have descriptions`,
      data: { sampled, withDescription },
    };
  }
  ...
  return {
    ...
    label,
    status: "fail",
    detail: `Only ${withDescription} of ${sampled} sampled fields have descriptions`,
    data: { sampled, withDescription },
  };
}
